...
{#if noteId}
  {#if connectionStatus === 'connected' && isEncrypted}
    <div class="inline-flex items-center gap-2 translate-y-0.5">
  ...
  {:else if connectionStatus === 'connected'}
    <div class="inline-flex items-center gap-2">
      <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse translate-y-0.5" title="Real-time sync active - Your changes are synced instantly"></div>
      <span class="inline-block text-xs text-muted-foreground translate-y-px">
        {clientId.substring(0, 4)}{#if connectedUsers.size > 1} +{connectedUsers.size - 1}{/if}
      </span>
    </div>
  {:else if connectionStatus === 'connecting'}
    <div class="inline-flex items-center translate-y-0.5">
