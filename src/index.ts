...
function rateLimit(limit: number, windowMs: number = 60000) {
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    const sessionId = c.req.query("session_id") || c.req.header("X-Session-ID") || "anonymous";
...
app.get("/api/notes/:id/ws", rateLimit(RATE_LIMITS.API.WS_UPGRADE_PER_MINUTE), async (c) => {
  ...
  const note = await c.env.DB.prepare("SELECT id, expires_at FROM notes WHERE id = ?")
    .bind(id)
    .first();
  ...
  await c.env.DB.prepare("UPDATE notes SET last_accessed_at = ? WHERE id = ?")
    .bind(Date.now(), id)
    .run();
...
app.get("/api/notes/:id", rateLimit(RATE_LIMITS.API.READ_PER_MINUTE), async (c) => {
  ...
  if (!note.is_encrypted) {
    ...
    c.env.DB.prepare("UPDATE notes SET view_count = ?, last_accessed_at = ? WHERE id = ?")
      .bind(newViewCount, Date.now(), id)
      .run()
      .catch((error: any) => {
    ...
    if (isLastView) {
      c.env.DB.prepare("DELETE FROM notes WHERE id = ?")
        .bind(id)
        .run()
        .catch((error: any) => {
    ...
    c.env.DB.prepare("UPDATE notes SET last_accessed_at = ? WHERE id = ?")
      .bind(Date.now(), id)
      .run()
      .catch((error: any) => {
  ...
  const doResponse = await stub.fetch(
    new Request(`http://do/fetch?noteId=${encodeURIComponent(id)}`, { method: "GET" }),
  );
...
app.post("/api/notes/:id/view", rateLimit(RATE_LIMITS.API.READ_PER_MINUTE), async (c) => {
  ...
  // Update view count
  await c.env.DB.prepare("UPDATE notes SET view_count = ? WHERE id = ?")
    .bind(newViewCount, id)
    .run();
...
app.post("/api/notes", rateLimit(RATE_LIMITS.API.CREATE_PER_MINUTE), async (c) => {
  ...
  if (id) {
    ...
    const existing = await c.env.DB.prepare("SELECT id FROM notes WHERE id = ?").bind(id).first();
  ...
  if (max_views !== undefined && max_views !== null) {
    if (max_views < LIMITS.MIN_VIEWS || max_views > LIMITS.MAX_VIEWS) {
      return c.json(
        {
          error: `max_views must be between ${LIMITS.MIN_VIEWS} and ${LIMITS.MAX_VIEWS.toLocaleString()}`,
        },
        400,
      );
    }
...
app.put("/api/notes/:id", rateLimit(RATE_LIMITS.API.UPDATE_PER_MINUTE), async (c) => {
  ...
  const expires_at = clear_expiration
    ? null
    : expires_in
      ? Date.now() + expires_in
      : existing.expires_at;
  ...
  const isSettingMaxViews =
    max_views !== undefined && max_views !== null && max_views !== existing.max_views;
  ...
  if (!result) {
    ...
    const current = await c.env.DB.prepare("SELECT version FROM notes WHERE id = ?")
      .bind(id)
      .first();
  ...
  const encryptionChanged =
    is_encrypted !== undefined && !!is_encrypted !== !!existing.is_encrypted;
  ...
  if (encryptionChanged) {
    ...
    try {
      ...
      await stub.fetch(
        new Request(`http://do/notify-encryption-change`, {
          method: "POST",
  ...
  if (finalIsEncrypted && content !== undefined) {
    try {
      ...
      await stub.fetch(
        new Request(`http://do/notify-version-update`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
...
app.post("/api/notes/:id/test-latency", async (c) => {
  ...
  try {
    ...
    const response = await stub.fetch(
      new Request(`http://internal/test-latency`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
app.delete("/api/notes/:id", rateLimit(RATE_LIMITS.API.DELETE_PER_MINUTE), async (c) => {
  ...
  try {
    ...
    await stub.fetch(
      new Request(`http://internal/reset`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
...
const scheduled: ExportedHandlerScheduledHandler<Env> = async (_event, env, _ctx) => {
  ...
  try {
    ...
    console.log(
      `Cron job: Cleaned up ${expiredResult.meta.changes} expired notes at ${new Date(now).toISOString()}`,
    );
    ...
    console.log(
      `Cron job: Cleaned up ${inactiveResult.meta.changes} inactive notes at ${new Date(now).toISOString()}`,
    );
  } catch (error) {
