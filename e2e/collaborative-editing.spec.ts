...
test.describe("Plaintext Mode - Cursor Preservation", () => {
  test("Cursor stays in place when remote user types AFTER cursor position", async ({
    browser,
  }) => {
  ...
  test("Cursor shifts left when remote user deletes BEFORE cursor position", async ({
    browser,
  }) => {
    const { clients, cleanup } = await setup2Clients(browser, "Hello World");
...
test.describe("Plaintext Mode - Desktop/Mobile Concurrent Editing", () => {
  test("CRITICAL: Cursor should NOT jump to end during concurrent editing", async ({ browser }) => {
    ...
    try {
      ...
      for (let round = 1; round <= 3; round++) {
        console.log(
          `Round ${round}: ${round === 3 ? "Typing with backspaces..." : "Both typing simultaneously..."}`,
        );
        ...
        if (desktopJumpedToEnd) {
          throw new Error(
            `Desktop cursor jumped to end! Position: ${desktopCursor}, Doc length: ${docEnd}`,
          );
        }
        if (mobileJumpedToEnd) {
          throw new Error(
            `Mobile cursor jumped to end! Position: ${mobileCursor}, Doc length: ${docEnd}`,
          );
        }
      }
  ...
  test("Scroll position maintained during remote edits at distant location", async ({
    browser,
  }) => {
    const longContent = generateLongContent(100);
    const { desktop, mobile, cleanup } = await setupDesktopMobileClients(browser, longContent);
  ...
  test("Content convergence after heavy concurrent editing", async ({ browser }) => {
    const { desktop, mobile, cleanup } = await setupDesktopMobileClients(
      browser,
...
test.describe("Syntax Highlight Mode - Desktop/Mobile Concurrent Editing", () => {
  test("CRITICAL: Cursor should NOT jump to end during concurrent editing in syntax mode", async ({
    browser,
  }) => {
    const codeContent = generateCodeContent(100);
    ...
    try {
      ...
      for (let round = 1; round <= 3; round++) {
        ...
        if (desktopJumpedToEnd) {
          throw new Error(
            `SYNTAX MODE: Desktop cursor jumped to end! Position: ${desktopCursor}, Doc length: ${docEnd}`,
          );
        }
        if (mobileJumpedToEnd) {
          throw new Error(
            `SYNTAX MODE: Mobile cursor jumped to end! Position: ${mobileCursor}, Doc length: ${docEnd}`,
          );
        }
      }
...
test.describe("Multi-Client Cursor Preservation", () => {
  ...
  test("Can continue typing while remote users edit at different positions", async ({
    browser,
  }) => {
